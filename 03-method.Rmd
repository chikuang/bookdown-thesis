# Numerical Algorithm and Applications {#chapter-applications}

\newcommand{\btheta}{\mathbf{\theta}}

We have studied the optimality conditions and the number of support points for optimal designs under SLSE in Chapter \@ref(chapter-SLSE). However, it is still challenging to find optimal designs analytically, except for simple regression models. Often numerical algorithms are used to compute optimal designs, and various algorithms have been studied including @titterington1978estimation, @silvey1978an, @dette2008improving, and @torsney2009multiplicative. For optimal designs under the SLSE, @bose2015optimal used multiplicative algorithms for finding the optimal designs whereas @gao2017d used convex programming and semi-definite programming (SDP) in MATLAB. Here we want to further extend the computational strategies for finding optimal designs under the SLSE. In this chapter, we discuss an effective algorithm to compute optimal designs based on CVX programming in MATLAB.
...
(\#eq:optimization)
\end{equation}
where $\phi$ is a loss function, which can be $\phi_A$, $\log(\phi_D)$ or $\phi_c$ defined in \eqref{eq:loss-B}. Notice that for any $\xi \in \Xi_N$, $\boldsymbol{B}(\xi,\boldsymbol{\theta}_o,t)=\mathbb{E}_{\xi}[\boldsymbol{M(\boldsymbol{x},\boldsymbol{\theta}_o},t)]=\sum_{i=1}^N \boldsymbol{M}(\boldsymbol{u}_i,\boldsymbol{\theta}_o,t)w_i$, where $\boldsymbol{M}(\boldsymbol{u_i},\boldsymbol{\theta}_o,t)$ is defined in \@ref(eq:M-matrix). We also note that a design point is selected if its weight is greater than a very small number $\delta_1$  (a small positive number, say $10^{-5}$) in practice. Lastly, we report the $d_D^{\max}=\max\{d_D |\boldsymbol{x}\in S\}$ ($d_A^{\max},~d_c^{\max}$) for D-optimality (A-, c-optimality), where $d_D,d_A$ and $d_c$ are defined in \@ref(eq:dire). If $d_D^{\max}<\delta_2$ (a small positive number, say $10^{-4}$), then the numerical solution to the above optimization problem is a D-optimal design by Theorem \@ref(thm:dispersion). Similarly we can check for A- and c-optimal designs. Problem \@ref(eq:optimization) is a convex optimization problem. We will discuss an algorithm to solve \@ref(eq:optimization) in the next section.

## Computational Algorithm {#section-algorithm}
There are some numerical algorithms which have been studied under the SLSE. @bose2015optimal applied multiplicative algorithms for finding the optimal designs to binary design points. @gao2017d computed D-optimal designs using the moments of distribution $\xi \in \Xi$. @yin2018optimal formulated the design problems as convex optimization problem and applied convex optimization programs CVX and SeDuMi in MATLAB to find D- and A-optimal designs, respectively. In their work, the A-optimal design problems are solved by SeDuMi which is hard to write the code in MATLAB. In our newly proposed formulation in \@ref(eq:optimization), we are able to use CVX to find A-optimal designs, which is much easier to code. Moreover, the CVX programming can be also applied to find c-optimal designs, which the previous algorithms were not applied before.
...
::: {.example #gompertz}
This example is for Gompertz growth model which was briefly described in Chapter \@ref(chapter:introduction) to showcase the output. Here, we use this model to show the effects of N on the D-optimal designs under SLSE with $\boldsymbol{\theta_o}=(1,1,1)^\top$, $S=[0.0,10.0]$ and $t=0$.
...
\begin{figure}
        ...
        \begin{subfigure}[b]{0.475\textwidth}
            ...
            \caption[]%
            {{\small}} 
            \includegraphics[width=\textwidth]{Figures/D_poly_5_t_000.jpg}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.475\textwidth}  
            \centering
            \caption[]%
            {{\small}} 
            \includegraphics[width=\textwidth]{Figures/D_poly_5_t_070.jpg}
        \end{subfigure}
        \vskip\baselineskip
        \begin{subfigure}[b]{0.475\textwidth}   
            \centering
            \caption[]%
            {{\small}} 
            \includegraphics[width=\textwidth]{Figures/A_poly_5_t_000.jpg}
        \end{subfigure}
        \quad
        \begin{subfigure}[b]{0.475\textwidth}   
            \centering
            \caption[]%
            {{\small}} 
            \includegraphics[width=\textwidth]{Figures/A_poly_5_t_070.jpg}
        \end{subfigure}
